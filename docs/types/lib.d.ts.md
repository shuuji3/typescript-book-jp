...
場合によっては、コンパイルターゲット(生成されたJavaScriptのバージョン)とアンビエントライブラリサポートの関係を切り離したい場合があります。2016年6月において一般的な例は、`Promise`です。たいていは`--target es5`のようにすることを望むと思いますが、しかし、それでも、`Promise`のような最新の機能を使いたい場合です。これをサポートするために、`lib`コンパイラオプションを使って`lib`を明示的に制御することができます。
